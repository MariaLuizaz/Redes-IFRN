...
def menu():
    ...
    while True:
        ...
        opcao = int(input)(f'\'Escolha uma das opções:')

        if opcao == '1':
            gerar = int(input('Quantas cartelas deseja gerar? '))
            if 1 <= gerar <= 10000:
            ...
            numero_cartela = input('Informe o número da cartela para imprimir (ex: CART_000123): ')
            ImprimirCartelas(cartelas, numero_cartela)
            print('Sorteio!')
            time.sleep(1)
...
def sortearDezenas(cartelas):
    dezenas_sort = random.sample(range(1, 76), 25)
    print(f'\nDezenas sorteadas: {dezenas_sort}')

    while True:
        cartelas_bat = []
        for numero_cartela, cartela in cartelas.items():
            numeros_cartela = cartela['B'] + cartela['I'] + cartela['N'] + cartela['G'] + cartela['O']
            if all(numero in dezenas_sort for numero in numeros_cartela):
                numero_cartela_batida = numero_cartela.split('_')[1]
                cartelas_bat.append(numero_cartela_batida)

        if cartelas_bat:
            print('\nCartelas batidas:')
            tabela_cartelas = [[num.center(6)] for num in cartelas_bat]
            print(tabulate(tabela_cartelas, headers=['Número da Cartela'], tablefmt='fancy_grid'))
            break
        else:
            nova_dezena = random.choice([x for x in range(1, 76) if x not in dezenas_sort])
            dezenas_sort.append(nova_dezena)
